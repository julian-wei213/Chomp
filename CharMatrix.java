...
public class CharMatrix
{
    ...
    public void fillRect(int row0, int col0, int row1, int col1, char fill) {
        if(row1 >= m.length) row1 = m.length-1;
        if(col1 >= m.length) col1 = m[0].length-1;
    ...
    public static void main (String[] args) {
        ...
        System.out.println("The number of rows is " + matrix.numRows());
        System.out.println("The number of columns is " + matrix.numCols());
        
        System.out.println("Testing fillRect .... ");
        matrix.display();
        matrix.fillRect(0,0,3,4,'k');
        matrix.display();
        matrix.clearRect(0,0,3,4);
        matrix.display();
    }
